...
`.trim();
    try {
        const raw = await completeStructuredJson({
            ...
            schema: refinamentoJsonSchema,
            temperature: 0.35,
            maxTokens: 1536
        });
        const parsed = safeParse(raw, { perguntas: [] });
        ...
        const normalizado = normalizarRefinamentoResponse(corrigirRefinamentoResposta(parsed), rodada);
