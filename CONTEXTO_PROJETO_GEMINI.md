...
```
src/app/
├── core/          # Models, services, guards, constants, data (catálogos/fallbacks)
├── pages/         # Telas do fluxo (7 componentes de página)
├── shared/        # Componentes reutilizáveis (car-3d, lgpd-notice)
├── app.module.ts
...
`TriageStateService` (`src/app/core/services/triage-state.service.ts`):
...
6 zonas fixas em `src/app/core/data/sintomas.catalog.ts`:
...
**Fallback local:** `src/app/core/data/refinamento.fallback.ts` — usado se API falhar ou timeout (45s).
...
- Fallback local: `src/app/core/data/cdp.fallback.ts`
...
Badges de urgência/confiança/prioridade em `src/app/core/constants/cdp-display.ts`.
...
Editar `sintomasPorZona` em `src/app/core/data/sintomas.catalog.ts`.
...
1. `src/app/core/models/cdp.model.ts` (TypeScript)
...
1. `src/app/core/constants/lgpd.constants.ts`
...
*Documento gerado em julho/2026. Para atualizar, regenere a partir do estado atual do repositório.*
