 ...
 * Usado pelo frontend e pelas APIs serverless.
 */

/** Substituições de expressão (ordem importa). */
const FRASES: ReadonlyArray<[RegExp, string]> = [
  [/\bnao\b/gi, 'não'],
  [/\bvoce\b/gi, 'você'],
  [/\bvc\b/gi, 'você'],
  [/\btambem\b/gi, 'também'],
...
const CORRECOES: Readonly<Record<string, string>> = {
  nao: 'não',
  voce: 'você',
  tambem: 'também',
  ate: 'até',
...
const EXCECOES_SUFIXO_AO = new Set([
  'cao',
  'mao',
  'pao',
  'tao',
...
function chaveSemAcento(palavra: string): string {
  return palavra
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '');
...
function temAcento(palavra: string): boolean {
  return /[àáâãäåèéêëìíîïòóôõöùúûüçñÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÇÑ]/.test(palavra);
}

function aplicarCapitalizacao(original: string, corrigida: string): string {
  if (original === original.toUpperCase()) return corrigida.toUpperCase();
...
function aplicarSufixos(chave: string): string {
  let r = chave;
...
export function corrigirOrtografiaPtBr(texto: string): string {
...
export function corrigirValorProfundo(valor: unknown): unknown {
  if (typeof valor === 'string') return corrigirOrtografiaPtBr(valor);
  if (Array.isArray(valor)) return valor.map(corrigirValorProfundo);
  if (valor && typeof valor === 'object') {
    const obj = valor as Record<string, unknown>;
    const next: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(obj)) next[k] = corrigirValorProfundo(v);
    return next;
  }
  return valor;
}
